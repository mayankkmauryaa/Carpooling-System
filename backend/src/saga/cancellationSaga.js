...
async function validateBookingStep(context) {
  try {
    ...
    logger.error('Cancellation Saga: validateBookingStep error', { error: error.message });
...
async function updateBookingStatusStep(context) {
  try {
    ...
    context.previousStatus = booking.status;
    ...
    logger.error('Cancellation Saga: updateBookingStatusStep error', { error: error.message });
    return { success: false, error: error.message };
  }
}
...
async function releaseSeatsStep(context) {
  try {
    ...
    logger.error('Cancellation Saga: releaseSeatsStep error', { error: error.message });
    return { success: false, error: error.message };
  }
}
...
async function processRefundStep(context) {
  try {
    ...
    logger.error('Cancellation Saga: processRefundStep error', { error: error.message });
    return { success: false, error: error.message };
  }
}
...
async function sendNotificationStep(context) {
  try {
    ...
    logger.error('Cancellation Saga: sendNotificationStep error', { error: error.message });
    return { success: false, error: error.message };
  }
}
...
async function completeSagaStep(context) {
  try {
    ...
    logger.error('Cancellation Saga: completeSagaStep error', { error: error.message });
    return { success: false, error: error.message };
  }
}
...
const cancellationCompensationSteps = [
  new SagaStep('validateBooking', async () => ({ success: true }), async () => ({ success: true })),
  new SagaStep('restoreBookingStatus', async (context) => {
    await prisma.booking.update({
      where: { id: context.bookingId },
      data: {
        status: context.previousStatus || 'CONFIRMED',
        cancelledAt: null,
        cancelledBy: null,
    ...
    logger.info('Cancellation Saga: Restored booking status', { bookingId: context.bookingId });
    return { success: true };
  }, async (context) => {
    await prisma.booking.update({
      where: { id: context.bookingId },
      data: {
        status: 'CANCELLED',
        sagaState: 'COMPENSATION_COMPLETE'
      }
    });
    return { success: true };
  }),
  new SagaStep('restoreSeats', async (context) => {
    ...
    logger.info('Cancellation Saga: Restored seats', { ridePoolId: context.ridePoolId, seats: context.seats });
    return { success: true };
  }, async (context) => {
    await prisma.ridePool.update({
      where: { id: context.ridePoolId },
  ...
  new SagaStep('processRefund', async () => ({ success: true }), async () => ({ success: true })),
  new SagaStep('sendNotification', async () => ({ success: true }), async () => ({ success: true })),
  new SagaStep('completeSaga', async () => ({ success: true }), async () => ({ success: true }))
];
